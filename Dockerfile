...
RUN uv pip install --system --no-cache . && uv pip install --system --no-cache --compile-bytecode -U "babeldoc @ git+https://github.com/TopCircle/BabelDOC.git@main" "pymupdf>=1.26.7" && babeldoc --version && babeldoc --warmup
RUN pdf2zh --version
CMD ["pdf2zh", "--gui"]
